...
fn computeMain(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    // density_reference is the coverage a uniform spread of the particles over the whole
    // domain would produce, so anything at or above "average" saturates to solid snow and
    // thinner cover fades out.
    let density = clamp(f32(count) / max(settings.density_reference, 1.0), 0.0, 1.0);

    // Thin cover reads as pale blue, dense deposit as opaque white.
    let sparse = vec3f(0.42, 0.62, 0.86);
    let dense = vec3f(1.0, 1.0, 1.0);
    let color = mix(sparse, dense, sqrt(density));
    let alpha = clamp(0.25 + 0.70 * density, 0.0, 0.95);
